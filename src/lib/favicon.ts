...
const renderFaviconToCanvas = (hue: number): Promise<Blob> => {
  return new Promise((resolve) => {
    ...
    const color = toHSLString(getAccentHSL(hue));
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><rect width="640" height="640" rx="128" fill="black"/><path fill="${color}" d="${ASTRONAUT_PATH}"/></svg>`;
...
export const updateFavicon = async (hue: number): Promise<void> => {
  ...
  const blob = await renderFaviconToCanvas(hue);
